...
class Printer():
    def __init__(self,url):
         ...
         img=qrcode.make(self._urlshort[0])
         mode = 'RGB' 
         size = (200, 200)
         color = (255, 255, 255)
         im = Image.new(mode, size, color)
         img = img.resize((75,75), Image.ANTIALIAS)
         im.paste(img, (int(200/2-100/2), int(200/2-100/2)))
         self._qrcode_logo='/home/pi/qrcode.png'
         im.save(self._qrcode_logo)
         self.url = url
         ...
         self._out=[(self._title2[i:i+n]) for i in range(0, len(self._title2), n)]
...
def get_url(url1):
     c.execute("SELECT * FROM database1 WHERE urls=?",(url1,))
     result=(c.fetchall())
     print(url1)
     if str(result)!='[]':
         print('already print')
         ...
         c.execute(" INSERT INTO database1 VALUES (?)",(url1,))
     connection.commit()   
